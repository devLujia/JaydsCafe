                ...
                {/* Cards top container */}
                 <div className='w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-5'>
                    ...
                    <div className='bg-white rounded-lg p-5 font-semibold shadow-lg dark:bg-gray-400'>
                        ...
                        <p className='text-2xl font-bold dark:text-gray-800'>₱ {((weeklytotalOrder?.totalOrders) * 15).toFixed(2) || 0}</p>
                    </div>
                    {/* Cards 3 */}
                    <div className='bg-white rounded-lg p-5 font-semibold shadow-lg dark:bg-gray-400'>
                        <div className='flex justify-between mb-5 dark:text-gray-100'>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="p-4 bg-white rounded-xl shadow-lg h-screen w-full dark:bg-gray-900 dark:text-gray-300 overflow-hidden overflow-y-auto ">
                        ...
                        <div className='w-full h-fit mt-4 space-y-5 '>
                            {/* pending 1 */}
                            {orders.map(order => (<div class="group w-full bg-white border-2 border-solid hover:bg-gray-100 border-gray-300 rounded-2xl p-6 transition-all duration-300 hover:border-greenColor">
                                <div key={order?.order_id} class="relative flex items-center gap-5 mb-6">
                                ...
                                <div className='flex flex-row shrink-0 gap-2'>
                                    <img src={clock}></img>
                                    <span class="text-sm leading-6 text-gray-500"> {order.order_date}</span>
                                </div>

                                <div className='mt-5 flex justify-between flex-wrap gap-y-4'>
                                    {/* <button onClick={()=>navigate('/RiderInbox')} className='font-semibold inline-flex justify-center items-center border border-solid border-slate-300 py-2 px-3 rounded-lg gap-2 text-slate-700'>
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-4 h-screen overflow-hidden dark:bg-gray-900 dark:text-gray-300">
                        ...
                        <div class="overflow-y-auto max-h-full pb-2 mt-5">
                            {orderId.map(messages => (<div class="relative flex items-center mb-4 cursor-pointer hover:bg-gray-100 p-2 rounded-md">
                                <div key={messages.order_id} class="w-12 h-12 bg-gray-300 rounded-full mr-3">
                                    <img src={jaydsLogo} alt="User Avatar" class="w-12 h-12 rounded-full"/>
                                </div>
                                <div class="flex-1">
                                    <h2 class="text-lg font-semibold">Customer name: {messages?.name}</h2>
                                    <p class="text-gray-600">Order No. {messages?.order_id}</p>
